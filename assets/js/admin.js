...
(function () {
  ...
  function renderPrice() {
    ...
    $('#priceTitle').value = p.title || 'VÀNG BẠC ĐÁ QUÝ NHẬT ANH'; $('#priceUnit').value = p.unit || ''; $('#priceDate').value = p.date || vnDate(); $('#pricePrevDate').value = p.prevDate || shiftDay(p.date || vnDate(), -1);
  ...
  $('#savePrice').addEventListener('click', async () => {
    ...
    state.settings.prices = { title: $('#priceTitle').value.trim() || 'VÀNG BẠC ĐÁ QUÝ NHẬT ANH', unit: $('#priceUnit').value.trim() || 'nghìn đồng/chỉ', date, prevDate, updatedAt: new Date().toISOString(), rows };
